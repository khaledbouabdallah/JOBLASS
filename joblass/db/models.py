...
class SearchCriteria(BaseModel):
    ...
    job_title: str = Field(..., description="Intitulé du poste recherché")
    location: str = Field(..., description="Lieu du poste recherché")
    easy_apply_only: bool = Field(False, description="Candidature facile uniquement")
    remote_only: bool = Field(False, description="En télétravail seulement")

    # --- Salary range ---
    salary_min_k: Optional[int] = Field(
        None, description="Salaire minimum en milliers d’euros"
    )
    salary_max_k: Optional[int] = Field(
        None, description="Salaire maximum en milliers d’euros"
    )

    # --- Company rating ---
    min_company_rating: Optional[Literal[1, 2, 3, 4, 5]] = Field(
        None, description="Note minimale de l’entreprise (1–5)"
    )
    ...
    ] = Field(None, description="Date de publication")

    # --- Job type ---
    job_type: Optional[str] = Field(None, description="Type de poste (CDI, CDD, etc.)")

    # --- City (geo dependent) ---
    city: Optional[str] = Field(None, description="Ville choisie (géolocalisée)")

    # --- Industry ---
    industry: Optional[str] = Field(None, description="Secteur d’activité")

    # --- Professional domain ---
    domain: Optional[str] = Field(None, description="Domaine professionnel")

    # --- Seniority level ---
    seniority_level: Optional[str] = Field(None, description="Niveau d’ancienneté")

    # --- Company size ---
    company_size: Optional[str] = Field(None, description="Taille d’entreprise")
